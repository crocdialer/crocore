...
public:
  ...
  static constexpr NamedUUID random() { return NamedUUID(generator()()); }

  static constexpr NamedUUID from_string(const std::string_view &str)
  {
    if(auto parsed_uuid = uuids::uuid::from_string(str)) { return NamedUUID(*parsed_uuid); }
    return nil();
  }

  static constexpr NamedUUID from_name(const std::string_view &name, const uuids::uuid &namespace_uuid = {})
  {
    return NamedUUID(uuids::uuid_name_generator(namespace_uuid)(name));
  }
