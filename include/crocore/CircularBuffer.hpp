...
public:
    ...
    explicit CircularBuffer(uint32_t cap, Allocator allocator = Allocator())
        : m_stl_allocator(std::move(allocator)), m_array_size((int32_t) cap + 1), m_first(0), m_last(0),
          m_data(m_stl_allocator.allocate(m_array_size))
    {}

    // copy constructor
    CircularBuffer(const CircularBuffer &other)
        : m_stl_allocator(other.m_stl_allocator), m_array_size(other.m_array_size), m_first(other.m_first),
          m_last(other.m_last), m_data(m_stl_allocator.allocate(m_array_size))
    {
        memcpy(m_data, other.m_data, m_array_size * sizeof(T));
    }

    // move constructor
    CircularBuffer(CircularBuffer &&other) noexcept : CircularBuffer() { swap(*this, other); }

    ~CircularBuffer() { m_stl_allocator.deallocate(m_data, m_array_size); }
    ...
    public:
        ...
        iterator() : m_buf(nullptr), m_array(nullptr), m_ptr(nullptr), m_size(0) {}

        iterator(const iterator &the_other)
            : m_buf(the_other.m_buf), m_array(the_other.m_array), m_ptr(the_other.m_ptr), m_size(the_other.m_size)
        {}

        iterator(const CircularBuffer<T> *the_buf, size_t the_pos)
            : m_buf(the_buf), m_array(the_buf->m_data), m_ptr(the_buf->m_data + the_pos), m_size(the_buf->m_array_size)
        {}
    ...
    {
        std::swap(lhs.m_stl_allocator, rhs.m_stl_allocator);
